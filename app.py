...
def load_all_supported_files_from_db():
	...
	return docs, file_list, file_map
...
if 'docs' not in st.session_state:
	...
	docs, file_list, file_map = load_all_supported_files_from_db()
	embedded_files = []
	...
	for fname in all_files:
		if fname in file_map:
			all_docs.append(file_map[fname])
	...
	if new_files:
		# Only embed new files, then concatenate with previous embeddings
		new_texts = [file_map[f] for f in new_files]
		new_emb = embed_texts(new_texts, st.session_state.model)
